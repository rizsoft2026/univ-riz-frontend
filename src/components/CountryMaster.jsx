...
import { Plus, Search, Edit2, Trash2, Globe, CheckCircle, XCircle } from 'lucide-react';
import Modal from './Modal';
import ConfirmModal from './ConfirmModal';
import baseApi from '@/services/baseApi';

export default function CountryMaster({ countries = [], setCountries, showNotification }) {
  ...
  const { data: fetchedData, isSuccess } = useQuery({
    queryKey: ['countries'],
    queryFn: async () => {
      const response = await baseApi.get('/countries');
  ...
  }, [isSuccess, fetchedData, setCountries]);

  const createMutation = useMutation({
    mutationFn: (data) => baseApi.post('/countries', data),
    onSuccess: (res) => {
      showNotification(`Country "${res.data.data.country_name}" created successfully!`, 'success');
      queryClient.invalidateQueries({ queryKey: ['countries'] });
      setIsFormOpen(false);
    },
    onError: (err) => {
      showNotification(err.response?.data?.message || 'Error creating country', 'error');
    }
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => baseApi.put(`/countries/${id}`, data),
    onSuccess: (res) => {
      showNotification(`Country "${res.data.data.country_name}" updated successfully!`, 'success');
      queryClient.invalidateQueries({ queryKey: ['countries'] });
      setIsFormOpen(false);
    },
    onError: (err) => {
      showNotification(err.response?.data?.message || 'Error updating country', 'error');
    }
  });

  const deleteMutation = useMutation({
    mutationFn: (id) => baseApi.delete(`/countries/${id}`),
    onSuccess: () => {
    ...
    onError: (err) => {
      showNotification(err.response?.data?.message || 'Error deleting country', 'error');
      setIsDeleteOpen(false);
    }
  });
  ...
  const handleCreateOpen = () => {
    setEditingItem(null);
    setFormData({ countryName: '', countryCode: '', status: 'Active' });
    setErrors({});
    setIsFormOpen(true);
  };
  ...
  const validateForm = () => {
    ...
    if (!formData.countryName.trim()) {
      tempErrors.countryName = 'Country Name is required';
    }
    ...
    <div className="space-y-5 animate-in fade-in-50 duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gradient-to-r from-blue-600 via-blue-700 to-sky-600 p-6 rounded-2xl text-white shadow-lg shadow-blue-600/10">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h2 className="text-2xl font-bold tracking-tight">Country Master</h2>
            <p className="text-blue-100 text-xs sm:text-sm mt-0.5">Manage countries for geographic and address mapping.</p>
          </div>
        </div>
      ...
      {/* Filter Bar */}
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between bg-blue-100/80 dark:bg-slate-900 p-4 rounded-2xl border border-blue-200 dark:border-slate-800 shadow-xs">
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 text-sm bg-white dark:bg-slate-950/40 border border-white/60 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 text-slate-800 dark:text-white placeholder:text-slate-400 shadow-sm"
          />
        </div>

        <div className="flex items-center gap-1 p-1 bg-white/60 dark:bg-slate-800/60 rounded-xl shrink-0 shadow-sm">
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Country Name *</label>
            <input
              type="text"
              name="countryName"
              ...
              className={`w-full px-3.5 py-2.5 text-sm bg-slate-50 dark:bg-slate-950/40 border ${
                errors.countryName ? 'border-red-500' : 'border-slate-200 dark:border-slate-800'
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Country Code (Optional)</label>
            <input
              type="text"
              name="countryCode"
              value={formData.countryCode}
              onChange={handleInputChange}
              placeholder="e.g. IN or IND"
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
            ...
            >
              {editingItem ? 'Save Changes' : 'Create Country'}
            </button>
          </div>
        </form>
      </Modal>
